...
public class hard extends AppCompatActivity {
    ...
    private void showResult(){
        ...
        int highscore=settings.getInt("HARD_HIGH_SCORE",0);
        if(player1Points>highscore){
            highscore=player1Points;
            SharedPreferences.Editor editor=settings.edit();
            editor.putInt("HARD_HIGH_SCORE",player1Points);
            editor.commit();
        }
        else{
            highscore=highscore;
        }
        AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(hard.this);
        alertDialogBuilder
